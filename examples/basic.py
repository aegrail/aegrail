...
This shows the three primitives in action without any LLM
...
def main() -> None:
    ...
    try:
        with agent.session(user_id="alice", task="refund order 4521") as s:
            # Pretend we just made an LLM call.
            s.record_llm(
            ...
            result = s.call_tool(
                "refund_api.refund",
                refund,
                _arg_summary={"order_id": 4521},
                order_id=4521,
            )
            print("tool result:", result)

            # Another LLM turn — confirm to the user.
            s.record_llm(
                model="claude-sonnet-4-5",
                tokens_in=80,
                tokens_out=40,
